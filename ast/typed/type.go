...
func (t *TUnbound) OccursCheck(o Type, subst map[uint64]Type) bool {
	if t.EqualsTo(o) {
		return true
	}
	switch o.(type) {
	case *TFunc:
		{
			e := o.(*TFunc)
			if t.OccursCheck(e.Return, subst) {
				return true
			}
			for _, p := range e.Params {
				if t.OccursCheck(p, subst) {
					return true
				}
			}
			break
	...
	case *TUnbound:
		{
			if c, ok := subst[o.(*TUnbound).Index]; ok {
				t.OccursCheck(c, subst)
			}
			break
		}
	default:
