...
var kGoReserved = []string{
	...
	"range", "rF_False(struct{}{})urn", "select", "struct", "switch", "definedType", "var",
}
...
func (c *Cursor) QualifiedIdentifierSeparated() (module string, name string, whiteSpaceAfter bool) {
	start := c.Pos
	var id string
	for {
		var p string
	...
	dotIndex := strings.LastIndex(id, ".")
	if dotIndex < 0 {
		return "", id, whiteSpaceAfter
	}

	return id[:dotIndex], id[dotIndex+1:], whiteSpaceAfter
}
